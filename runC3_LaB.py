...
def extract_prediction(raw_output: str) -> str:
    ...
    2) Lines starting with letter A-D, optional punctuation → first occurrence
    ...
    for line in lines:
        ...
        # 2️⃣ Check if line starts with a letter A-D (optionally with punctuation)
...
BENCHMARK_DIR = "./Benchmarks/cosmosqa"
RESULTS_DIR = "./Results"
...
# ---------------- PROMPT ----------------
def build_prompt(question, options_dict, passage):
    options_text = ""
    for i in ALLOWED_POS:
        val = options_dict.get(i, "N/A")
    ...
    return (
        "Instruction: Solve the multiple choice question based on the given {passage}. "
        "ONLY write the answer in the format 'Answer: X'. "
        "Do not add explanations. Use uppercase letters A-D.\n\n"
        "Example 1:\n"
        "passage: I exited the plane as rear float , facing forward . One I began riding off the hill I became unstable and worked from my feet to my arms to regain the stablity . First I corrected my feet trying to keep them level and stable .\n"
        "Question: Why did you exit the plane from a float ? \n"
        "A:The pilot wanted to test out the floats and decided to use them\nB: The pilot was running a test landing and wanted to see how we did exiting via the floats\nC: The plane had to have an emergency landing and that meant they used an emergency landing procedure which involved the floats.\nD: None of the above choices \n" 	
        "Answer: C\n\n"
        "Example 2:\n"
        "passage: Oh no ! I went to Starbucks tonight to meet with a friend . I went early , you know any way I can get out of the house without the children and spend some quiet time on my own , works for me !.\n"
        "Question: What may be your reason for going to Starbucks ? \n"
        "A:I wanted a break from my family .\nB: The kids enjoy quiet time there .\nC: I invited someone there .\nD: My family wanted to go to Starbucks .\n" 	
        "Answer: A\n\n"
        "Task:\n"
        f"{passage_text}"
        f"Question: {question}\n"
...
def generate_label_bias_cases(df, seed=42):
    ...
    for _, row in df.iterrows():
        ...
        for target_correct_label in labels:
            ...
            final_case = {
                "id": row["id"],
                "passage": row.get("passage", ""),   # ✅ ADD THIS
...
def worker(npu_id, questions, progress_counter, output_path, model_path, max_tokens, debug=False):
    ...
    for idx, q in enumerate(questions):
        prompt = build_prompt(
            q["question"],
            {l: q[f"option{l}"] for l in ALLOWED_POS},
            q.get("passage", "")
        )
    ...
    for csv_file in os.listdir(BENCHMARK_DIR):
        ...
        for model_path in LLMs:
            ...
            for label, questions in all_cases.items():  # separate files per label
                ...
                os.system("pkill -9 -f python runC3_LaB.py")
